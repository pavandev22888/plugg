...
ipcMain.handle('install-plugin', async (event, pluginId: string, dawName: string) => {
  return installer.installPlugin(pluginId, dawName)
})
ipcMain.handle('uninstall-plugin', async (event, pluginId: string, dawName: string) => {
...
ipcMain.handle('download-marketplace-plugin', async (event, pluginId: string) => {
...
ipcMain.handle('rate-plugin', async (event, pluginId: string, rating: number) => {
  return marketplace.ratePlugin(pluginId, rating)
})
ipcMain.handle('delete-plugin', async (event, pluginId: string) => {
