...
class Detector {
  async getDAWPaths(): Promise<DAWConfig[]> {
  ...
  private async getWindowsPaths(): Promise<DAWConfig[]> {
    const daws: DAWConfig[] = []
    const vst3Path = 'C:\\Program Files\\Common Files\\VST3'

    const configs: { name: DAWConfig['name']; regPath: string }[] = [
  ...
  private async getMacPaths(): Promise<DAWConfig[]> {
    const daws: DAWConfig[] = []
    const vst3Paths = [
    ...
    const appPaths: { name: DAWConfig['name']; appPath: string }[] = [
